...
public class LVConsultarFacturasPagar extends JDialog implements MouseListener, ActionListener, ItemListener{
	   ...
	   // private JButton jbHistorial;
	    ...
	    public LVConsultarFacturasPagar(JFrame padre, Profesor e){
	    	super(padre);
	    	estado = true;
	    ...
	    public void init(boolean ban){
	        ...
	        jTable1 =  new JTable(factura){public boolean isCellEditable(int rowIndex, int colIndex) {
	        	return false;}};
	        	if(bprofesor.find(profesor.getProfId())){
	                 jtfNombreE.setText(profesor.getProfNombre()+" "+ profesor.getProfApellido1()+" "+profesor.getProfApellido2()); 
	                 this.jtfIDE.setText(profesor.getProfId());
	        	}
          ...
          /*  
			...
			*/
			...
			//List<FacturaEntrada> listFacturas = bprofesor.getFacturaSalida(profesor.getProfId());
			...
			B_CuentasPorCobrar bean= new B_CuentasPorCobrar();
       		List<CuentasPorCobrar> lista = bean.cuentasPorEstudiante(profesor.getProfId());
           ...
           for(int i=0;i<lista.size();i++){        	   
				String[] fila = {
						...
						profesor.getProfId(),
						profesor.getProfNombre() + " " + profesor.getProfApellido1(),

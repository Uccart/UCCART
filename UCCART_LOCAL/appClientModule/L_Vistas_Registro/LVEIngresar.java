...
public class LVEIngresar extends JDialog implements ActionListener {
	private Font fo;
    ...
    public LVEIngresar(JFrame padre, boolean modal, Font f){
		super(padre, modal);
		fo = f;
		estado = true;
	...
	public void init(){
		...
		jLabel5.setFont(fo);
		JLabel jLabel6 = new JLabel("Teléfono:");
		jLabel6.setFont(fo);
        ...
        jbagregar.setToolTipText("Agregar nuevo estudiante!");
		...
		if(listcarr.size() == 0){
			JOptionPane.showMessageDialog(null, "Deben crearse carreras antes de poder ingresar estudiantes", "Error", JOptionPane.ERROR_MESSAGE);
			this.dispose();
			estado = false;
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == jbagregar){
			if(!("".equals(jtfid.getText()) ) && !("".equals(jtfnombre.getText())&&!("".equals(jtfapellido1.getText()))&&!("".equals(jtfnacionalidad.getText()))
				...
				if(bestudiante.validapk(jtfid.getText())){
					...
					JOptionPane.showMessageDialog(null, "No se permite duplicar el ID en estudiante", "Error", JOptionPane.ERROR_MESSAGE);
				}
			}else{
